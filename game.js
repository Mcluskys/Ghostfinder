...
function drawPlaying(currentTime) {
  ...
  // Couche d'obscurité + halo de la lampe, équivalent au rendu Pygame.
  ctx.save();
  ctx.fillStyle = "rgba(0,0,0,0.86)";
  ctx.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
  ctx.globalCompositeOperation = "destination-out";
  const gradient = ctx.createRadialGradient(pointer.x, pointer.y, 0, pointer.x, pointer.y, LIGHT_RADIUS);
...
canvas.addEventListener("pointermove", event => {
  ...
  // Souris/stylet : suivi permanent. Tactile : suivi pendant le glissement.
