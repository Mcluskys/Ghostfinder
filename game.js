...
function improveMobilePlay() {
  // Ne force ni plein écran ni rotation : Safari/iOS et certains navigateurs
  // Android peuvent interrompre le geste tactile ou redimensionner le canvas
  // au moment du START. Le CSS responsive suffit pour jouer partout.
}
...
// ======================
// ENTREES SOURIS + TACTILE
// ======================
// Sur mobile, la lampe suit le doigt en continu pendant le glissement.
// On utilise Pointer Events lorsqu'ils sont disponibles, avec écoute globale
// des déplacements pour ne pas perdre le geste si le doigt sort du canvas.
...
function clientToLogical(clientX, clientY) {
  const rect = canvas.getBoundingClientRect();
  return {
    x: Math.max(0, Math.min(LOGICAL_W, (clientX - rect.left) * LOGICAL_W / rect.width)),
    y: Math.max(0, Math.min(LOGICAL_H, (clientY - rect.top) * LOGICAL_H / rect.height)),
  };
}

function setPointerFromClient(clientX, clientY, type = "mouse") {
...
function handlePress(clientX, clientY, type = "mouse") {
  const raw = clientToLogical(clientX, clientY);
  pointer.x = raw.x;
  pointer.y = raw.y;
  ...
  // Sur PC, le clic garde le comportement historique.
  if (state === "playing" && type !== "touch") tryFindGhost();
}

function handleMove(clientX, clientY, type = "mouse") {
  const raw = clientToLogical(clientX, clientY);
  if (pointer.down && distance(raw.x, raw.y, pointer.startX, pointer.startY) > 8) {
    pointer.moved = true;
  }
  // Important : sur smartphone la lampe suit exactement le doigt.
  // Aucun décalage n'est appliqué, ce qui évite l'impression de lampe bloquée.
  pointer.x = raw.x;
  pointer.y = raw.y;
  pointer.type = type;
}

function handleRelease(type = pointer.type) {
  // Un petit tap tactile agit comme un clic. Un glissement sert uniquement
  // à déplacer la lampe, puis on peut taper le fantôme quand il est éclairé.
  if (type === "touch" && state === "playing" && !pointer.moved) {
    tryFindGhost();
  }
  pointer.down = false;
}
...
if (window.PointerEvent) {
  canvas.addEventListener("pointerdown", event => {
    ...
    handlePress(event.clientX, event.clientY, event.pointerType || "mouse");
    try { canvas.setPointerCapture(event.pointerId); } catch {}
  }, { passive: false });

  // Écoute sur window : le déplacement continue même si le doigt dépasse
  // légèrement du canvas ou si Safari/Chrome change la cible du pointermove.
  window.addEventListener("pointermove", event => {
    if (event.pointerType === "touch") {
      if (activePointerId !== event.pointerId || !pointer.down) return;
      event.preventDefault();
    }
    handleMove(event.clientX, event.clientY, event.pointerType || "mouse");
  }, { passive: false });

  window.addEventListener("pointerup", event => {
    if (activePointerId !== null && event.pointerId !== activePointerId && event.pointerType === "touch") return;
    handleRelease(event.pointerType || pointer.type);
  ...
  // Fallback iOS/Safari ancien : suivi tactile natif par identifiant de doigt.
  canvas.addEventListener("touchstart", event => {
    ...
    touchMoved = false;
    handlePress(t.clientX, t.clientY, "touch");
  }, { passive: false });

  window.addEventListener("touchmove", event => {
    const touches = Array.from(event.touches);
    const t = touches.find(x => x.identifier === activeTouchId);
    if (!t) return;
    event.preventDefault();
    if (Math.hypot(t.clientX - touchStartClientX, t.clientY - touchStartClientY) > 8) touchMoved = true;
    handleMove(t.clientX, t.clientY, "touch");
  }, { passive: false });

  window.addEventListener("touchend", event => {
    const t = Array.from(event.changedTouches).find(x => x.identifier === activeTouchId);
    if (!t) return;
    event.preventDefault();
    pointer.moved = pointer.moved || touchMoved;
    handleRelease("touch");
    activeTouchId = null;
  }, { passive: false });
  ...
  canvas.addEventListener("mousedown", event => {
    handlePress(event.clientX, event.clientY, "mouse");
  });
  window.addEventListener("mousemove", event => {
    handleMove(event.clientX, event.clientY, "mouse");
  });
  window.addEventListener("mouseup", () => handleRelease("mouse"));
}
